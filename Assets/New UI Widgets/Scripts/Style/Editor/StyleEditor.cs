	...
	public class StyleEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (style.IsDefault())
			{
				...
				{
					style.SetAsNotDefault();
					serializedObject.FindProperty("Default").boolValue = false;
				}
			}
			else
			{
				...
				{
					style.SetAsDefault();
					serializedObject.FindProperty("Default").boolValue = true;
				}
			}
			...
			Fast.ForEach(x => EditorGUILayout.PropertyField(SerializedProperties[x], true));
			...
			Detailed.ForEach(x => EditorGUILayout.PropertyField(SerializedProperties[x], true));
