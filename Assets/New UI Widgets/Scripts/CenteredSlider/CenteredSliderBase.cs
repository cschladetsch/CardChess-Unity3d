namespace UIWidgets
{
	...
	[DataBindSupport]
	public abstract class CenteredSliderBase<T> : UIBehaviour, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler, IStylable
		where T : struct
	{
		/// <summary>
		/// OnChangeEvent
		...
		{
			var current_style = IsHorizontal() ? style.CenteredSliderHorizontal : style.CenteredSliderVertical;
			...
			if (handle != null)
			{
				current_style.Handle.ApplyTo(handle.GetComponent<Image>());
			}
