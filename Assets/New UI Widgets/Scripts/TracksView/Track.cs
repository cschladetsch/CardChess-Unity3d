namespace UIWidgets
{
		...
		where TPoint : IComparable
	{
		...
		/// <summary>
		/// Duplicate list comparison.
		/// </summary>
		protected readonly Comparison<TData> DuplicateComparison = (x, y) =>
		{
		...
		/// <summary>
		/// Comparison for the group.
		/// </summary>
		protected readonly Comparison<TData> GroupComparison = (x, y) =>
		{
		...
		protected void UpdateSortedData()
		{
			...
			SortedData.Sort(DuplicateComparison);
		}
		...
		protected void CalculateVisibleItems()
		{
			...
			var dragged_index = SortedData.FindIndex(x => x.IsDragged);
