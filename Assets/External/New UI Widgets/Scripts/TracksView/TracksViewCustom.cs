namespace UIWidgets
{
	...
	public abstract partial class TracksView<TData, TPoint, TDataView, TTrackView, TTrackBackground, TTrackDataDialog, TTrackDataForm, TTrackDialog, TTrackForm> : TracksViewBase<TData, TPoint>
		...
		where TTrackForm : TrackFormBase<TData, TPoint>
	{
		...
		{
			TrackDialog.Edit(track, x => x.CopyTo(track));
		}
		...
		{
			TrackDataDialog.Edit(data, x => DataCopy(x, data));
		}
	}
}