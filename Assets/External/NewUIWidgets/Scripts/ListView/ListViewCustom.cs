namespace UIWidgets
{
		...
		where TComponent : ListViewItem
	{
		...
		List<int> highlightedIndices = new List<int>(2);
		...
		{
			return ScrollToAnimatedCoroutine(() => ListRenderer.GetPosition(index), unscaledTime);
		}
		...
		public override void ComponentsColoring(bool instant = false)
		{
			...
			{
				ComponentsPool.ForEach(x => DefaultColoring(x as ListViewItem));
				return;
			}
		...
		public virtual void ChangeDefaultItemSize(Vector2 size)
		{
			...
			ComponentsPool.ForEachAll(item =>
			{
				var item_rt = item.transform as RectTransform;
		...
		/// Selectable data.
		/// </summary>
		protected struct SelectableData
		{
			/// <summary>
